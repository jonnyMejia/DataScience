function metodo_simpson_compuesto()
    ...
    if(bandera)
        ...
        if(mod(m,2)==0)%si la cantidad de puntos ingresados es par
            %Se separa un vector con los dos ultimos para hallar el valor del
            %area con el metodo del trapecio
            vector_valor_x_trapecio=vector_valores_x(m-1:m,:);
            vector_valores_fx_trapecio=vector_valores_fx(m-1:m,:);
            area_simpson=0;
            ...
            if(m>2) 
                ...
                vector_valor_x_simpson=vector_valores_x(1:m-1,:);
                vector_valores_fx_simpson=vector_valores_fx(1:m-1,:);
                area_simpson=metodo_simpson(m-1,vector_valor_x_simpson,vector_valores_fx_simpson);
            end
            ...
            area = metodo_simpson(m,vector_valores_x,vector_valores_fx);
        end
      
        %Se muestra el area calculada
      
        fprintf("El area del polinomio: '%s' mediante el metodo simpson compuesto de grado 2 es = %f\n",Titulo, area);
        ...
        if(mod(m,2)==0)%si la cantidad de puntos ingresados es par
            ...
            disp([vector_valores_x  vector_valores_fx]);
            disp(size(vector_valores_x,1));
            for i = 1:2:size(vector_valores_x,1)-2
                    %primero se define el dominio de x
                    dominio_x=vector_valores_x(i)-0.2*(vector_valores_x(i+2)-vector_valores_x(i)):0.001:vector_valores_x(i+2)+0.2*(vector_valores_x(i+2)-vector_valores_x(i));

                    %luego se define los puntos de la funcion a partir del dominio de xvector_valores_x,vector_valores_fx,
                    vector_funcional=polyval(minimos_cuadrados_parametrada(2,3,vector_valores_x(i:i+2,:),vector_valores_fx(i:i+2,:)),dominio_x);
